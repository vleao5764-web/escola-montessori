import desenhoErlenmeyer from "@/assets/desenhos/Desenho_06.png";
import desenhoMicroscopio from "@/assets/desenhos/Desenho_11.png";
      ...
      <div className="mx-auto max-w-[1280px] px-6">
        ...
        <div className="grid gap-10 lg:grid-cols-[65fr_35fr] lg:items-stretch">
          <div>
            ...
            <p className="mt-7 max-w-[58ch] text-base leading-relaxed text-campaign-deep-purple/80 sm:text-[1.05rem]">
              Desenvolvida por Maria Montessori, a metodologia nasceu da observação
              do desenvolvimento humano e da compreensão de que a aprendizagem é
              mais significativa quando o aluno participa ativamente do processo.
              Aqui, o ambiente é preparado para favorecer exploração, concentração e
              construção progressiva do conhecimento, e o educador atua como guia e
              observador.
            </p>
          </div>

          {/* painel único roxo profundo */}
          <div
            aria-hidden="true"
            className="relative min-h-[260px] overflow-hidden rounded-[28px] bg-campaign-deep-purple"
          >
            <img
              src={desenhoMicroscopio}
              alt=""
              className="absolute bottom-8 left-8 w-[120px]"
            />
            <img
              src={desenhoErlenmeyer}
              alt=""
              className="absolute right-8 top-8 w-[80px]"
            />
          </div>
        </div>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {AREAS.map((a) => (
            <article
              key={a.nome}
              className={`rounded-[20px] p-7 ${a.bg} ${a.fg}`}
            >
              <h4 className="font-display text-2xl font-extrabold uppercase leading-tight">
                {a.nome}
              </h4>
              <p className={`mt-3 text-[0.95rem] leading-relaxed ${a.sub}`}>
                {a.desc}
              </p>
            </article>
          ))}
